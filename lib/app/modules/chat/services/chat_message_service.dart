...
class ChatMessageService extends GetxService {
  ...
  RxList<Map<String, dynamic>> get recentChats =>
      _recentChats ??= RxList<Map<String, dynamic>>();

  RxMap<String, double>? _uploadProgress;
  RxMap<String, double> get uploadProgress =>
      _uploadProgress ??= RxMap<String, double>();
          ...
          for (var msgData in data) {
            ...
            if (msgData['content'] != null) {
              ...
              if (_decryptionCache.containsKey(cacheKey)) {
                ...
                debugPrint(
                  'Used cached decryption for real-time message: $messageId',
                );
              } else {
                try {
                  // Attempt to decrypt the message content
                  ...
                  msgData['is_encrypted'] = false;
                  debugPrint(
                    'Successfully decrypted real-time message: $messageId',
                  );
                } catch (e) {
                  debugPrint(
                    'Could not decrypt real-time message $messageId: $e',
                  );
                  // If decryption fails, leave content as is (might be plaintext or old format)
                }
              }
            ...
            if (msgData['sender_id'] == currentUserId) {
              debugPrint(
                'Skipping real-time update for self-sent message: $messageId',
              );
              return;
            }
  ...
  Future<void> _updateRecentChat(String chatId, String lastMessageText) async {
    try {
      ...
      if (chatIndex != -1) {
        ...
        recentChats[chatIndex]['last_message_time'] =
  ...
  Future<void> uploadAndSendAudio({
    ...
    try {
      ...
      final fileName =
          '${DateTime.now().millisecondsSinceEpoch}_${path.basename(localAudioPath)}';
  ...
  Future<void> uploadAndSendImage({
    ...
    try {
      ...
      final fileName =
          '${DateTime.now().millisecondsSinceEpoch}_${path.basename(localImagePath)}';
      final storageRef = '$chatId/$fileName';
      ...
      if (index != -1) {
        ...
        // CRITICAL: Force reactive update to ensure UI shows the image
        messages.refresh();

        // Also update the controller's messages list if it's different
        if (controller.messages != messages) {
          final controllerIndex = controller.messages.indexWhere(
            (m) => m.messageId == tempMessageId,
          );
          if (controllerIndex != -1) {
            controller.messages[controllerIndex] = updatedMessage;
            controller.messages.refresh();
          }
        }

        debugPrint('Image message updated in UI: ${sentMessage.messageId}');
      } else {
        debugPrint('Warning: Could not find temporary message to replace');
        // If we can't find the temp message, just add the new one
        messages.add(
          sentMessage.copyWith(content: imageUrl, isEncrypted: false),
        );
        controller.messagesToAnimate.add(sentMessage.messageId);
        messages.refresh();
      }

      // Force update UI lists and clear upload progress
      uploadProgress.remove(tempMessageId);
      messages.refresh();
      controller.messages.refresh();

      debugPrint('Image upload completed and UI updated');
  ...
  Future<void> cleanupExpiredMessages() async {
    ...
    try {
      ...
      final expiredMessages =
          messages.where((msg) => msg.expiresAt.isBefore(now)).toList();
